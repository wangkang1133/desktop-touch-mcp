  ...
  server.registerTool(
    "scroll",
    {
      description: buildDesc({
        ...
        caveats: "action='capture' returns stitched image — pixels do NOT match screen coords when sizeReduced=true, use for reading only, not mouse_click. action='smart' CDP path requires browser_open. action='to_element' native path requires element to implement UIA ScrollItemPattern. action='read' uses OCR (imperfect accuracy) and requires the window to be visible; for browser pages prefer browser_eval (e.g. evaluate document.body.innerText) or browser_overview to extract DOM text accurately. action='raw' typed errors: code:'ScrollNotDelivered' on silent drop (overlay window covering the target / non-scrollable container / UIPI low-IL → elevated app); already-at-page-boundary is treated as success via pre/post-percent disambiguation rather than a typed error. action='raw' hints.verifyDelivery.reason is migrating from a 4-value matrix-doc enum (read_back_unsupported / page_end_inferred / scrollbar_unavailable / no_target_window — current) to a 5-value tier-based enum (delivered_via_uia / delivered_via_cdp / delivered_via_postmessage / wheel_overlay_intercepted / target_unreachable — see docs/adr-018-input-pipeline-3tier.md §2.6.2). The 5 new reasons become emittable as ADR-018 Phase 1b/3/4 land their respective tier dispatchers. action='smart' typed errors: code:'OverflowHiddenAncestor' (retry with expandHidden:true), code:'VirtualScrollExhausted' (provide virtualIndex).",
        examples: [
