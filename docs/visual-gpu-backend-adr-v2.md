...
- [x] **4b-5**: Stage 1 (region proposer) → Stage 2 (UI detector) → Stage 3 (OCR) を直列で繋ぐ — **framework のみ** (各 stage は stub inference)。`session_pool.rs` (VisionSessionPool + global OnceLock)、`RecognizeRequest::session_key` field、`init_session_blocking` の pool insert、`recognize_rois_blocking` の session-bound stub path、`stage-pipeline.ts` (runStagePipeline 3-stage orchestrator)、`OnnxBackend.ensureWarm` (manifest load + 3 × visionInitSession)、`OnnxBackend.recognizeRois` (stage pipeline 経由)。実 preprocess/postprocess/session.run は sub-batch 4b-5a (Florence-2) / 4b-5b (OmniParser-v2) / 4b-5c (PaddleOCR-v4) で実装予定。cargo check 3 feature set / tsc noEmit / vitest 57 (stage-pipeline 6 + onnx-backend 12 + model-registry 22 + session 17) all pass、regression 0。詳細は `docs/phase4b-5-stage-pipeline-design.md`
