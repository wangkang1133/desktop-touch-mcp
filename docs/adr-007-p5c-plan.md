...
| **PR-P5c-2** | DXGI dirty rect emit (詳細 sub-plan: `docs/adr-007-p5c-2-plan.md`、ADR-008 D2 §3.bis ledger L1 trigger) | 中 | 200-250 line (sub-plan §4 で再見積) |
...
詳細 acceptance は **`docs/adr-007-p5c-2-plan.md` §7** (sub-plan)。本 §11.4 は最低限の親 acceptance のみ:

- [ ] 画面変化で `EventKind::DirtyRect` event が ring push
- [ ] 既存 napi polling と並行動作可
- [ ] DXGI 利用不可環境で graceful disable
- [ ] sub-plan §7.2 追加 acceptance (`enable_l1_emit` toggle / AccessLost spam 抑制 / `#[allow(dead_code)]` marker 削除) も全 pass
