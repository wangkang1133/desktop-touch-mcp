...
| Word `EnumChildWindows`-based descendant assertion (`_WwG` / `_WwO` confirmed under `OpusApp`) | Phase 5+N follow-up PR (chain extended from Phase 5 trunk) | ~~Requires new `win32_enum_child_windows` napi export.~~ **Updated 2026-05-15**: the runtime dispatcher fix landed via the lighter-shaped `win32_find_scroll_leaf_for_top_level` (class-chain table over `FindWindowExW` — see `docs/adr-018-phase-5-followup-leaf-walker-subplan.md`), so the Word `_WwG` runtime contract no longer requires a generic enumerator. The smoke-test descendant assertion is now a documentation-only artefact that can re-target the new helper (or read `OpusApp` direct children via the same `FindWindowExW` pattern) when the 4-app smoke harness expansion lands. Phase 4 fixture remains as skeleton (top-level enumeration only). (PR #305 Opus Round 1 P1-1 → PR #306 Opus Round 2 P1-C → PR fix/adr-018-excel-scroll-leaf-walker chain.) |
| Word real-app integration assertion (scroll actually moves Word document via Tier 3) + Excel cell area Tier 3 real-app smoke + Explorer ListView Tier 3 real-app smoke | Phase 5+N follow-up PR (chain extended from Phase 5 trunk) | Phase 4 contributes the Tier 3 dispatcher wire-up + sub-plan §2.3/§2.4 contract pins + the Word fixture skeleton only. Phase 5 trunk PR #306 honestly reduced scope to 3 deliverables and re-routed the 4-app smoke expansion (`scroll-5app.smoke.test.ts` Notepad/Word/Excel/Explorer) to Phase 5+N. (PR #305 Opus Round 1 P2-2 → PR #306 Opus Round 2 P1-C chain.) |
| `wheel_overlay_intercepted` detection (DDPM-style invisible overlay sensor) | Future / OQ2 | ADR §7 OQ2; not gated on Phase 4. |
| Shared `findPlainTopLevelWindowByTitle` helper | **Landed in Phase 5 trunk PR #306** | Phase 1b §2.2 originally committed Phase 4 to extract; re-routed Phase 4 → Phase 5 (PR #305 Opus Round 1 P1-2). Phase 5 sub-plan §2.1#2 landed the extraction with a `{excludeMinimized, excludeDialogsAndOwned}` option object preserving each call site's behaviour bit-equal. 9 unit cases in `tests/unit/find-plain-top-level-window.test.ts`. ✅ Done. |
