...
| Word `EnumChildWindows`-based descendant assertion (`_WwG` / `_WwO` confirmed under `OpusApp`) | Phase 5+N follow-up PR (chain extended from Phase 5 trunk) | Requires new `win32_enum_child_windows` napi export. Phase 4 fixture lands as skeleton (top-level enumeration only). Phase 5 trunk PR #306 did not land the descendant assertion (no new Rust); the item remains in Phase 5 sub-plan §2.2 carry-over for a follow-up PR. (PR #305 Opus Round 1 P1-1 → PR #306 Opus Round 2 P1-C chain.) |
