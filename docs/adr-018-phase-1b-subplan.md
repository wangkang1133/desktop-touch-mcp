...
| Shared `findPlainTopLevelWindowByTitle` helper (de-dup the Case 3 title→HWND predicate) | Phase 5 | `resolveInputDestination` (Round 3/4) applies `_resolve-window.ts` Case 3's `plainMatch` constraints inline (non-dialog class + no owner) plus a minimized-window exclusion, and `scrollHandler` keeps a separate observation-ladder predicate (minimized + title only, no dialog/owner filter). Two/three copies of one "title → HWND" rule — Codex Round 3 P2 / Round 4 P1 + Opus Round 3 P2-1. **Re-routed Phase 4 → Phase 5 (PR #305 Opus Round 1 P1-2)** because Phase 4 trunk focused on Tier 3 PostMessage wire-up and the predicate-extraction work has no Phase 4 contract dependency. Phase 5 extracts a shared `findPlainTopLevelWindowByTitle` in `_resolve-window.ts` so the dialog/owner predicate has one SSOT and cannot drift. **The shared helper MUST keep `resolveInputDestination`'s minimized-window exclusion as a distinct opt-in (e.g. an `{ excludeMinimized }` flag) — `_resolve-window.ts` Case 3 itself does NOT filter minimized, so collapsing the two predicates verbatim would re-introduce Round 4 P1 (a minimized HWND recovered as a dispatch target).** Not a correctness bug today. |
