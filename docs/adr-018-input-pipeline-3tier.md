...
- `observation?:` — **ADR-019 MVP-1 additive field** carrying a `VisualMotionObservation` (Temporal Motion Observation Layer / TMOL primitive output). The canonical `source` enum is defined in **`docs/adr-019-anti-fukuwarai-v3-temporal-motion-observation.md` §2.1** (single source of truth, **9-value enum** post-Stage-5: `uia_scroll_percent` / `block_motion_vectors` / `tiled_phase_correlation` / `ssim_residual` / `dxgi_dirty_rect` / `dxgi_dirty_rect_unavailable` / `optical_flow` / `temporal_ring_observation_only` / `chain_trust_unverified`). Existing callers that ignore the field are unaffected. The field is populated when a TMOL primitive produced an observation: Stage 1 UIA `ScrollPercent` pre/post observation on the `postWheelToHwnd` chain-trust branch emits `uia_scroll_percent` / `chain_trust_unverified`; **Stage 4** (sub-plan `docs/adr-019-stage-4-plan.md`) emits `source: "ssim_residual"`, wired into `mouse_click.verifyDelivery` (`focus_only` / `unverifiable` fallback) and `keyboard:type` BG verify (`unverifiable + read_back_unsupported` fallback); **Stage 5** (sub-plan `docs/adr-019-stage-5-plan.md`) emits `source: "dxgi_dirty_rect"` (or `"dxgi_dirty_rect_unavailable"` for RDP / `NotCurrentlyAvailable` graceful-degrade), wired primarily into `desktop_act` post-state via `hints.verifyDelivery.observation` and optionally as an env-gated safety net (`DESKTOP_TOUCH_STAGE5_DXGI_FALLBACK=1`) for `mouse_click` / `keyboard:type` when Stage 4 returns `indeterminate` with no `residual`. Absent on legacy Tier 1 / 2 / 3 paths that have not yet been wired through TMOL.
