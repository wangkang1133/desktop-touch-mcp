...
| 16 | `view_update_latency` p99 < 1ms SLO の達成可否 (D2-A 実測 3.0ms、D1 baseline 4.7ms から 1.5× 改善ながら未達) | **Pending — Opus 諮問判断 2026-05-02 で案 (2) SLO 文言正確化採択**: option C-2/C-3 (release semantics 再設計) は永久 defer (`feedback_auto_progress_d2_option_c.md`)。判断結論: views-catalog §3.1 SLO を 4 種 (operator step / lookup / release-to-view / MCP round-trip) に分解、release-to-view は N3 partial-order contract により `shift_ms` 下限が構造的必然として明示、production 観測 SLO は MCP round-trip で評価。precise 文言確定の prerequisite として **D2-B-5 (`docs/adr-008-d2-b-5-plan.md`)** で operator-induced view-hit MCP round-trip 数値取得中、本数値で SLO `<TS p99 / 5` か `<TS p99 / 1.5` か `<10ms` のいずれかが finalize。Resolved 化は後続 PR-2 (views-catalog 4 種分解 + 8 SSOT 同期) で実施 |
