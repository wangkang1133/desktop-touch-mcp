...
|---|---|---|---|---|
| L1 | D2-C `dirty_rects_aggregate` view (`crates/engine-perception/src/views/dirty_rects_aggregate.rs` + `src/l3_bridge/dirty_rect_pump.rs`) | ADR-007 **P5c-2** (DXGI dirty rect → L1 ring `EventKind::DirtyRect` emit site 配線、`docs/adr-007-p5c-2-plan.md` sub-plan) | **PR-ε (D2-C)** を P5c-2 完了の翌 PR で起動 (§D2-C-1〜D2-C-4 の checklist そのまま) | P5c-2 PR で Rust integration test (mock context-based、`spawn(0)` + `Next` cmd 経由) で `EventKind::DirtyRect` の `ring.push` を pin (Notepad/Edge fixture-based live induce は P5c-2 sub-plan §5 follow-up へ carry-over)、その上で D2-C で view test (out-of-order frame_id partial-order test 等) |
