...
| 11 | **Resolved (PR #86, 2026-04-30)** — L1 worker `shutdown_with_timeout` (l160-184) と `shutdown_l1_for_test` (l211-224) の同型 shutdown bug 2 件: (a) handle を unconditional に take してから helper-thread 経由 join、timeout 後に handle が helper thread 内 stale (b) `shutdown_l1_for_test` が `inner.shutdown_with_timeout()` 前に slot clear、timeout 後 `ensure_l1()` が二重 thread spawn | High | (a) `JoinHandle::is_finished()` polling 化で handle が timeout 後も recoverable に / (b) `Arc::clone` borrow + Ok 時のみ slot clear。さらに Codex PR #86 P2 で Ok arm に `Arc::ptr_eq` guard を追加 (並行 shutdown が new worker の slot を消す race を閉じる)。発見: PR #84 Codex review v5+v6 で UIA 側を fix、L1 worker は scope 分離で R11 として持ち越し → PR #86 で closure |
| 12 | **Resolved (本 PR、2026-04-30)** — UIA `shutdown_uia_for_test` (l154-183) の Ok arm に R11 / PR #86 と同型 race が残っていた (clone した `inner_arc` の shutdown 成功後、slot が同じ Arc を指すか確認せず `*guard = None`)。並行 shutdown で新 COM thread の slot を古い shutdown 呼び出しが消し、UIA singleton + apartment-affinity invariant が崩れる可能性 | Medium (test-only path、ただし P5c-1 で event handler dropper として本番 shutdown ordering でも呼ばれる) | L1 と同形 `Arc::ptr_eq` guard を horizontal port。発見: PR #86 Codex review (R11 closure 後の水平展開抜け指摘) |
